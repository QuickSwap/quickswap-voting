...
contract RiboseX is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 totalAmountTokensFee; // Total amount of tokens fee that collected from users withdrawing
    ...
    uint256 public feePercent = 1; // fee for withdrawing
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        pool.stakingTokenTotalAmount -= _amount;
        user.rewardDebt = user.amount * pool.accYelPerShare / 1e12;
        (amountAfterFee_, feeAmount_) = _getAmountAfterFee(_amount);
        pool.totalAmountTokensFee += feeAmount_;
        pool.stakingToken.safeTransfer(address(msg.sender), amountAfterFee_);
        emit Withdraw(msg.sender, _pid, amountAfterFee_);
    }
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        (amountAfterFee_, feeAmount_) = _getAmountAfterFee(userAmount);
        pool.totalAmountTokensFee += feeAmount_;
        pool.stakingToken.safeTransfer(address(msg.sender), amountAfterFee_);
        emit EmergencyWithdraw(msg.sender, _pid, amountAfterFee_);
    }
    ...
    function _getAmountAfterFee(uint256 amount) internal view returns(uint256, uint256) {
        /*
        ...
        */
        uint256 feeAmount = amount.mul(feePercent).mul(100).div(10000);
        return (amount.sub(feeAmount), feeAmount);
    }

    function withdrawAllTokensFee(address owner, uint256 pid) onlyOwner public {
        PoolInfo storage pool = poolInfo[pid];
        uint256 tokenBalance = pool.stakingToken.balanceOf(address(this));
        require(
            pool.totalAmountTokensFee <= tokenBalance,
            "RiboseX: the token balance of the contract should be equal or more then pool.totalAmountTokensFee"
        );
        pool.stakingToken.transfer(owner, pool.totalAmountTokensFee);
        pool.totalAmountTokensFee = 0;
    }
}